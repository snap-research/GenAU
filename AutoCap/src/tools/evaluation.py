...
from ACT.coco_caption.pycocotools.coco import COCO
from ACT.coco_caption.pycocoevalcap.eval import COCOEvalCap
...
def evaluate_metrics_from_files(pred_file: Union[Path, str],
                                ref_file: Union[Path, str], exclude_metrics=[]) \
    ...
    cocoEval = COCOEvalCap(coco, cocoRes, exclude_metrics=exclude_metrics)
...
def evaluate_metrics_from_lists(predictions: List[str],
    ...
    metrics, per_file_metrics = evaluate_metrics_from_files(pred_file, ref_file, exclude_metrics=exclude_metrics)
...
def BERTScore(model, gt_captions, pred_captions, batch_size=16):
    gt_embeddings = np.concatenate([model.encode(gt_captions[i:i+batch_size]) for i in range(0, len(gt_captions), batch_size)], axis=0)
    pred_embeddings = np.concatenate([model.encode(pred_captions[i:i+batch_size]) for i in range(0, len(pred_captions), batch_size)], axis=0)

    similarities = np.einsum('ij,ij->i', gt_embeddings, pred_embeddings) / (np.linalg.norm(gt_embeddings, axis=1) * np.linalg.norm(pred_embeddings, axis=1))
...
def evaluate_metrics(prediction_file: Union[str, Path, List[Dict[str, str]]],
    ...
    metrics, per_file_metrics = evaluate_metrics_from_lists(predictions, ground_truths, exclude_metrics=exclude_metrics)
    ...
    #     with torch.no_grad():
    #         bert_score = BERTScore(bert_model, [c[0] for c in ground_truths], predictions)
    #     bert_model = bert_model.cpu()
    #     total_metrics['bert_score'] = bert_score
