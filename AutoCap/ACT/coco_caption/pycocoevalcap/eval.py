...
class COCOEvalCap:
    def __init__(self, coco, cocoRes, exclude_metrics=[]):
        self.evalAudios = []
        self.eval = {}
        self.audioToEval = {}
        self.coco = coco
    ...
    def evaluate(self, verbose=True):
        ...
        print("self.exclude_metrics", self.exclude_metrics)
        # exclude metrics
        scorers = [score for score in scorers if (not isinstance(score[1], str)) or (score[1].lower() not in self.exclude_metrics)]

        print("scores", scorers)

        # =================================================
        # Compute scores
        # =================================================
        for scorer, method in scorers:
            # if method == 'SPICE' or method == 'METEOR':
            #     continue
            if verbose:
        ...
        if 'cider' not in self.exclude_metrics and 'spice' not in self.exclude_metrics:
            if verbose:
