...
from src.modules.diffusionmodules.attention import LinearAttention
...
class AttnBlock1D(nn.Module):
    ...
    def forward(self, x):
        # assuming input x: B x C x L
        h_ = x
        h_ = self.norm(h_)
        q = self.q(h_)
        k = self.k(h_) # b x c x l
        v = self.v(h_)
        ...
        k = k # b x c x l
        w_ = torch.bmm(q, k).contiguous()  # b, l, l    w[b,i,j]=sum_c q[b,i,c]k[b,c,j]
        w_ = w_ * (int(c) ** (-0.5))
...
class Encoder1D(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        # end
        self.norm_out = Normalize(block_in)
        print("latent dim", z_channels, 2 * z_channels if double_z else z_channels)
