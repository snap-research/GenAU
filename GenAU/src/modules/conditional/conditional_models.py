...
from src.modules.clap.open_clip import create_model
from src.modules.clap.training.data import get_audio_features
...
from src.modules.audiomae.AudioMAE import Vanilla_AudioMAE
from src.modules.phoneme_encoder.encoder import TextEncoder
...
from src.modules.audiomae.sequence_gen.model import CLAP2AudioMAE
from src.modules.audiomae.sequence_gen.sequence_input import (
    Sequence2AudioMAE,
)
...
class FlanT5HiddenState(nn.Module):
    # TODO: decide on how to pass the device
    """
    ...
    def __init__(
        ...
        return_embeds=False, pool_tokens=False,
    ):
        super().__init__()
    ...
    def get_unconditional_condition(self, batchsize):
        ...
        if self.empty_hidden_state_cfg is None:
            if self.return_embeds:
                self.empty_hidden_state_cfg = self([""])
            else:
    ...
    def encode_text(self, prompt):
        ...
        return [
            outputs.encoder_hidden_states.detach(),
            attention_mask.float(),
        ]  # Attention mask == 1 means usable token

class LAIONCLAP(nn.Module):

    def __init__(self, sampling_rate, embed_mode, freeze_text_encoder=True):
        super().__init__()
        self.freeze_text_encoder = freeze_text_encoder
    ...
    def get_unconditional_condition(self, batchsize):
        self.unconditional_token = self.model.get_text_embedding(["", ""])[0:1]
        ...
        if self.embed_mode == "audio":
            ...
            with torch.no_grad():
                ...
                if self.sampling_rate != 48000:
                    print("batch before res", batch.shape)
                    batch = torchaudio.functional.resample(
                        batch, orig_freq=self.sampling_rate, new_freq=48000
                    )
                    print("batch after res", batch.shape)
                ...
                # )
                # [bs, 512]
                embed = self.model.get_audio_embedding_from_data(batch, use_tensor=True)
        elif self.embed_mode == "text":
            with torch.no_grad():
                # the 'fusion' truncate mode can be changed to 'rand_trunc' if run in unfusion mode
                if len(batch) == 1:
                    embed = self.model.get_text_embedding([batch[0], "text"], use_tensor=True)[:1]
                else:
                    embed = self.model.get_text_embedding(batch, use_tensor=True)

        embed = embed.unsqueeze(1)

        # Disable decision for now
...
class DatasetIDs(nn.Module):
    # TODO: decide on how to pass the device
    def __init__(self, dataset2id:dict, encode_dataset_ids:bool=False):
        super().__init__()
        if encode_dataset_ids:
            print("[warning] encoding dataset ids is not tested, if you are using the FIT backbone, please disable encode_dataset_ids as it will be handlded automatically")
    
        self.dataset2id = dataset2id
        self.encode_dataset_ids = encode_dataset_ids
        
        if self.encode_dataset_ids:
             self.dataset_token_embed = nn.Embedding(len(self.dataset2id.keys()), self.dataset_embed_dim) 
        
    def forward(self, batch, device='cuda'):
        # add dataset ids to cond_dict
        dataset_ids = []
        if 'dataset_name' in batch:
            for dataset_name in batch['dataset_name']:
                if dataset_name not in self.dataset2id.keys():
                    print(f"[Warning] can't find mapping for dataset {dataset_name}, using id 0 by default")
                dataset_ids.append(self.dataset2id.get(dataset_name, 0))
            if not self.encode_dataset_ids:
                return torch.Tensor(dataset_ids).long().to(torch.int64).to(device)
            else:
                return self.dataset_token_embed(torch.Tensor(dataset_ids).long().to(torch.int64).to(device))
        else:
            print("[WARNINF] conditioning on dataset ids is enabled but dataset_name attribute is not found, using dataset id as zero by default")
            dataset_ids = torch.zeros(len(batch['fname'])).long().to(torch.int64).to(device)
            if not self.encode_dataset_ids:
                return torch.Tensor(dataset_ids).long().to(torch.int64).to(device)
            else:
                return self.dataset_token_embed(torch.Tensor(dataset_ids).long().to(torch.int64).to(device))
            
    def get_unconditional_condition(self, batchsize, device='cuda'):
        dataset_ids = torch.zeros(batchsize).long().to(torch.int64).to(device) * self.dataset2id.get('unconditional', 0)
        if not self.encode_dataset_ids:
            return torch.Tensor(dataset_ids).long().to(torch.int64).to(device)
        else:
            return self.dataset_token_embed(torch.Tensor(dataset_ids).long().to(torch.int64).to(device))
...
class CLAPAudioEmbeddingClassifierFreev2(nn.Module):
    def __init__(
        ...
        print("self.embed_mode", self.embed_mode)
    ...
    def forward(self, batch):
        ...
        if self.embed_mode == "audio":
            if not self.training:
                print("[Warning]: clap model calculate the audio embedding as condition")
            with torch.no_grad():
                # assert (
                #     self.sampling_rate == 16000
                # ), "We only support 16000 sampling rate"
...
if __name__ == "__main__":
    model = CLAPAudioEmbeddingClassifierFreev2(
        pretrained_path="/fsx/mali6/repos/AudioLDM2-training/data/checkpoints/clap_htsat_tiny.pt",
        embed_mode="text",
        amodel="HTSAT-tiny",
    ).cuda()
    audio_path = '/fsx/mali6/repos/AudioLDM2-training/data/dataset/audioset/zip_audios/unbalanced_train_segments/unbalanced_train_segments_part0/Y-_2oJZ-NhQg.wav'
    text = ["A muffled sound that seems like wind is heard as there is another sharp sound"]

    wavform, sr = torchaudio.load(audio_path)
    # model = LAIONCLAP()
    # data = torch.randn((6, 1, int(16000*10.24)))
    data = ["text"]
    res = model(data)
    print("similarity", model.cos_similarity(wavform.cuda(), text))
    # import ipdb
