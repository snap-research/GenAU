...
def copy_test_subset_data(metadata, testset_copy_target_path):
    ...
    print("[INFO] Copying test subset data to {}".format(testset_copy_target_path))
    print("[INFO] Length of test subset is:", len(metadata))
    for i in tqdm(range(0, len(metadata))):
        each = metadata[i]
        file_path = each['fname'] if 'fname' in each else each['wav']
        cmd = "cp {} {}".format(file_path, os.path.join(testset_copy_target_path))
        os.system(cmd)
