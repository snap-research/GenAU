...
class EvaluationHelper:
    def __init__(self, sampling_rate, device, backbone="cnn14") -> None:
        ...
        # load clap model 
        # self.laion_model = LAIONCLAP(sampling_rate=self.sampling_rate, embed_mode="audio")
        # self.laion_model = self.laion_model.to(device) # download the default pretrained checkpoint.
    ...
    def get_filename_intersection_ratio(
        ...
        if (
            ...
            print(
                "+Two path  %s and %s have %s intersection files out of total %s & %s files. Processing two folder with same_name=True"
                % (dir1, dir2, len(intersect_keys), len(keyset1), len(keyset2))
            )
            return True
        else:
            print(
                "-Two path %s and %s  have %s intersection files out of total %s & %s files. Processing two folder with same_name=False"
                % (dir1, dir2, len(intersect_keys), len(keyset1), len(keyset2))
            )
            return False
    ...
    def calculate_metrics(self, generate_files_path, groundtruth_path, same_name, limit_num=None, calculate_psnr_ssim=False, calculate_lsd=False, recalculate=False):
        ...
        laion_clap_score = 0
        # all_clap_sim = None
        # for waveform, name in outputloader:
            # print("name", name)
            # print("waveform". waveform.shape)
            # 
            # text = self.test_dataset_captions[name]
            # print("text", text)
            # similarity = self.clap.cos_similarity(
            #             torch.FloatTensor(waveform).squeeze(1).cuda(), [text]
            #         )
            # clap_score += torch.mean(similarity)
            ...
            #             torch.FloatTensor(waveform).squeeze(1).cuda(), [text]
            #         )
            # laion_clap_score += torch.mean(similarity)
