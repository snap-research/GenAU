...
class Cnn14(nn.Module):
    def __init__(
        ...
        cache_dir = os.getenv("CACHE", f"{home_dir}/.cache")
        if not os.path.exists("%s/audioldm_eval/ckpt/Cnn14_mAP=0.431.pth" % (cache_dir)):
            print("Download pretrained checkpoints of Cnn14.")
            os.makedirs("%s/audioldm_eval/ckpt" % (cache_dir), exist_ok=True)
            os.system("wget -P %s/audioldm_eval/ckpt/ %s" % (cache_dir,"https://zenodo.org/record/3576403/files/Cnn14_mAP%3D0.431.pth"))
            os.system("wget -P %s/audioldm_eval/ckpt/ %s" % (cache_dir,"https://zenodo.org/record/3987831/files/Cnn14_16k_mAP%3D0.438.pth"))

        # self.init_weight()
        if sample_rate == 16000:
            state_dict = torch.load("%s/audioldm_eval/ckpt/Cnn14_16k_mAP=0.438.pth" % cache_dir)
            self.load_state_dict(state_dict["model"])
        elif sample_rate == 32000:
            state_dict = torch.load("%s/audioldm_eval/ckpt/Cnn14_mAP=0.431.pth" % cache_dir)
            self.load_state_dict(state_dict["model"])
...
class Cnn14_16k(nn.Module):
    def __init__(
        ...
        home_dir = os.path.expanduser("~")
        cache_dir = os.getenv("CACHE", f"{home_dir}/.cache")
        if not os.path.exist("%s/audioldm_eval/ckpt/Cnn14_mAP=0.431.pth" % cache_dir):
